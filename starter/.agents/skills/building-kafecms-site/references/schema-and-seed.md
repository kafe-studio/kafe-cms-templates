...
```json
{
	"$schema": "https://kafecms.com/seed.schema.json",
	"version": "1",
	"meta": {
		"name": "My Site",
...
Collections define content types. Each collection becomes a database table (`kc_{slug}`).
...
{
	"id": "byline-editorial",
	"slug": "kafecms-editorial",
	"displayName": "KafeCMS Editorial"
}
```
...
Use `$media` for image fields -- KafeCMS downloads and stores the image:
...
The seed at `.kafecms/seed.json`, `package.json#kafecms.seed`, or `seed/seed.json` is inlined into the build and applied on the first request when the database is empty and the setup wizard hasn't been completed. Existing data is never overwritten.
...
```bash
npx kafecms export-seed                      # Schema only
npx kafecms export-seed --with-content       # Schema + all content
npx kafecms export-seed --with-content=posts,pages  # Specific collections
```
