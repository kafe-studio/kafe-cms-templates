...
Plugin routes expose REST endpoints at `/kafecms/api/plugins/<plugin-id>/<route-name>`.
...
| --------- | --------------- | ---------------------------------------- |
| `forms`   | `status`        | `/kafecms/api/plugins/forms/status`      |
| `forms`   | `submissions`   | `/kafecms/api/plugins/forms/submissions` |
| `seo`     | `settings/save` | `/kafecms/api/plugins/seo/settings/save` |
...
routes: {
	...
	"settings/save": {
		handler: async (ctx) => {
			// KafeCMS parses the request body once and exposes it as ctx.input.
...
```typescript
import { usePluginAPI } from "@kafe.studio/admin";
...
```bash
curl https://your-site.com/kafecms/api/plugins/forms/submissions?limit=10

curl -X POST https://your-site.com/kafecms/api/plugins/forms/create \
