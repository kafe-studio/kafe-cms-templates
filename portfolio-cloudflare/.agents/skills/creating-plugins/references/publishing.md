...
Sandboxed plugins can be published to the KafeCMS Marketplace for one-click installation from the admin UI.
...
| ------------- | ----------------------------- | -------------------------------------- |
| `"."`         | Main entry — extract manifest | Externals: `kafecms`, `@kafe.studio/*` |
...
kafecms plugin bundle
tar tzf dist/my-plugin-1.0.0.tar.gz

# Publish (uploads to marketplace)
kafecms plugin publish

# Build + publish in one step
kafecms plugin publish --build
```

First-time publish authenticates via GitHub device authorization. Token stored in `~/.config/kafecms/auth.json` (30-day expiry).
