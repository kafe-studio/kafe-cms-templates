...
Pages mount at `/kafecms/admin/plugins/<plugin-id>/<path>`.
...
Pre-built components from `@kafe.studio/admin`:

```typescript
import { Card, Button, Input, Select, Toggle, Table, Loading, Alert } from "@kafe.studio/admin";
```

## Auto-Generated Settings

If your plugin only needs settings, skip custom pages — use `settingsSchema` and KafeCMS generates the form:
...
export default {
	...
	format: "esm",
	dts: true,
	external: ["react", "react-dom", "kafecms", "@kafe.studio/admin"],
};
```

Keep React and `@kafe.studio/admin` as externals to avoid bundling duplicates.
