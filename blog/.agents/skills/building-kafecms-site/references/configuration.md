...
```bash
KAFECMS_SITE_URL=https://mysite.example.com
...
Every KafeCMS site needs this file at `src/live.config.ts`. It's boilerplate -- the same in every project:
...
This registers KafeCMS's live content collections with Astro. All content types are served through the single `_kafecms` collection -- you query specific types using `getKafeCMSCollection("posts")` etc.
...
import type { PortableTextBlock } from "kafecms";
...
```bash
# From local dev server (writes kafecms-env.d.ts at project root)
npx kafecms types
...
The CLI also writes `.kafecms/schema.json` with the raw schema for tooling.
...
For Cloudflare, replace `@astrojs/node` with `@astrojs/cloudflare` and add `@kafe.studio/cloudflare`.

## Dev Server

```bash
npx kafecms dev              # Start dev server (runs migrations, applies seed)
npx kafecms dev --types      # Start and generate types from schema
```

The admin UI is at `http://localhost:4321/kafecms/admin`. On first run, you'll go through setup to create an admin account.
