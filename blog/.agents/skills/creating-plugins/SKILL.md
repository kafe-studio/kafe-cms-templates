...
description: Create KafeCMS CMS plugins with hooks, storage, settings, admin UI, API routes, and Portable Text block types. Use this skill when asked to build, scaffold, or implement an KafeCMS plugin, or when creating plugin features like custom block types, admin pages, or content hooks.
---

# Creating KafeCMS Plugins

KafeCMS plugins extend the CMS with hooks, storage, settings, admin UI, API routes, and custom Portable Text block types. All plugins are TypeScript packages.

## Plugin Types

KafeCMS has two plugin formats:
...
import type { PluginDescriptor } from "kafecms";
...
import { definePlugin } from "kafecms";
import type { PluginContext } from "kafecms";
...
KafeCMS has two execution modes. Plugin code is identical in both — only the enforcement changes.
...
Standard plugins can be published to the KafeCMS Marketplace for one-click installation:

```bash
kafecms plugin bundle --dir packages/plugins/my-plugin  # creates .tar.gz
kafecms plugin login                                      # authenticate via GitHub
kafecms plugin publish --tarball dist/my-plugin-1.0.0.tar.gz
```
...
Configure `package.json` exports so KafeCMS can load each entry point:
...
| **API Routes**      | `definePlugin({ routes })`   | Yes      | Yes    | REST endpoints at `/kafecms/api/plugins/<id>/<route>` |
